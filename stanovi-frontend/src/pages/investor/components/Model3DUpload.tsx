...
export function Model3DUpload({ onUploadSuccess, isLoading = false }: Model3DUploadProps) {
  ...
  const validateFile = (file: File): { valid: boolean; error?: string } => {
    // Browsers rarely set a reliable mimetype for GLB, so validate by extension.
        ...
        <div className="flex flex-col items-center gap-2">
          {isLoading ? (
            ...
            <>
              ...
              <p className="text-xs text-gray-500">GLB • Max 10MB</p>
            </>
          )}
        </div>
      </div>
